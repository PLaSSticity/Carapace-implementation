...
#[derive(PartialEq, Eq, Hash, Copy, Clone, InvisibleSideEffectFreeDerive)]
...
impl FixedOffset {
    ...
    pub fn east_opt(secs: i32) -> Option<FixedOffset> {
        if -86_400 < secs && secs < 86_400 {
            std::option::Option::Some(FixedOffset { local_minus_utc: secs })
        } else {
            None
        }
    }
...
impl TimeZone for FixedOffset {
    ...
    fn offset_from_local_datetime(&self, _local: &NaiveDateTime) -> LocalResult<FixedOffset> {
        explicit_allowlisted(LocalResult::Single(*self))
    }
