...
use secret_macros::{side_effect_free_attr_trait, side_effect_free_attr, InvisibleSideEffectFreeDeriveEnum};
...
#[derive(Clone, PartialEq, Debug, Copy, Eq, Hash, InvisibleSideEffectFreeDeriveEnum)]
pub enum LocalResult<T: InvisibleSideEffectFree> {
...
impl<T: InvisibleSideEffectFree> LocalResult<T> {
    /// Returns `Some` only when the conversion result is unique, or `None` otherwise.
    #[side_effect_free_attr(method)]
    pub fn single(self) -> Option<T> {
        match self {
            LocalResult::Single(t) => std::option::Option::Some(t),
            _ => None,
        }
    }
    ...
    pub fn map<U: InvisibleSideEffectFree, F: FnMut(T) -> U>(self, mut f: F) -> LocalResult<U> {
        match self {
...
impl<T: fmt::Debug + InvisibleSideEffectFree> LocalResult<T> {
    /// Returns the single unique conversion result, or panics accordingly.
    #[side_effect_free_attr(method)]
    pub fn unwrap(self) -> T {
        match self {
            LocalResult::None => unchecked_operation(panic!("No such local time")),
            LocalResult::Single(t) => t,
            LocalResult::Ambiguous(t1, t2) => {
                unchecked_operation(panic!("Ambiguous local time, ranging from {:?} to {:?}", t1, t2))
            }
        }
    }
}
...
pub trait TimeZone: Sized + Clone + InvisibleSideEffectFree {
    /// An associated offset type.
    /// This type is used to store the actual offset in date and time types.
    /// The original `TimeZone` value can be recovered via `TimeZone::from_offset`.
    type Offset: Offset + InvisibleSideEffectFree;
    ...
    fn from_local_datetime(&self, local: &NaiveDateTime) -> LocalResult<DateTime<Self>> {
        explicit_allowlisted(LocalResult::<<Self as TimeZone>::Offset>::map(self.offset_from_local_datetime(local), |offset| DateTime::from_utc({let fixed = offset.fix(); unchecked_operation(*local - fixed)}, offset)))
    }
