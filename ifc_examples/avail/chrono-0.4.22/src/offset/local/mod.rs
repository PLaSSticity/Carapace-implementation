...
/// ```
#[derive(Copy, Clone, Debug, InvisibleSideEffectFreeDerive)]
...
impl TimeZone for Local {
    ...
    fn offset_from_local_datetime(&self, local: &NaiveDateTime) -> LocalResult<FixedOffset> {
        explicit_allowlisted(LocalResult::<DateTime<Local>>::map(self.from_local_datetime(local), |datetime| *datetime.offset()))
    }
    ...
    #[side_effect_free_attr(method)]
    fn from_local_datetime(&self, local: &NaiveDateTime) -> LocalResult<DateTime<Local>> {
        explicit_allowlisted(inner::naive_to_local(local, true))
    }
