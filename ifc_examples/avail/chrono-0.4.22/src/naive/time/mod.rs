...
use secret_macros::{InvisibleSideEffectFreeDerive, side_effect_free_attr};
...
#[derive(PartialEq, Eq, Hash, PartialOrd, Ord, Copy, Clone, InvisibleSideEffectFreeDerive)]
...
impl NaiveTime {
    ...
    pub fn from_hms_nano_opt(hour: u32, min: u32, sec: u32, nano: u32) -> Option<NaiveTime> {
        ...
        std::option::Option::Some(NaiveTime { secs, frac: nano })
    }
    ...
    fn hms(&self) -> (u32, u32, u32) {
        let (mins, sec) = explicit_allowlisted(div_mod_floor(self.secs, 60));
        let (hour, min) = explicit_allowlisted(div_mod_floor(mins, 60));
        (hour, min, sec)
    }
