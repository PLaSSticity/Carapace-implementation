...
#[derive(Clone, InvisibleSideEffectFreeDerive)]
...
impl<Tz: TimeZone> DateTime<Tz> {
    ...
    pub fn date(&self) -> Date<Tz> {
        Date::from_utc(self.naive_local().date(), unchecked_operation(self.offset.clone()))
    }
    ...
    pub fn time(&self) -> NaiveTime {
        let timed = self.datetime.time();
        let fixed = self.offset.fix();
        unchecked_operation(timed + fixed)
    }
    ...
    pub fn naive_local(&self) -> NaiveDateTime {
        let fixed = self.offset.fix();
        unchecked_operation(self.datetime + fixed)
    }
