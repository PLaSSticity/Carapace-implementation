...
use crate::datetime::availability;
use crate::datetime::{
    availability::{
...
use crate::events::{self, google, microsoft, Calendar, Event, GetResources};
use crate::store::{AccountModel, CalendarModel, Platform, Store, PLATFORMS};
use crate::util::AvailConfig;

use secret_structs::{secret::*, untrusted_secure_block_dynamic_secrecy, trusted_secure_block_dynamic_secrecy};
...
pub(crate) async fn find_availability(
    ...
    let mut tasks: Vec<JoinHandle<anyhow::Result<SecureValue<Vec<Event>, sec_lat::Label_Empty, int_lat::Label_All, DynLabel<Sec>, ()>>>> = vec![];
    
    let mut total_duration: std::time::Duration = std::time::Duration::ZERO;
    for account in accounts {
        let account_id = account.id.unwrap().to_owned();
    ...
    let mut AVAIL_TIMING: Instant = Instant::now();
    pub fn secure_flatten_and_collect(events: Vec<SecureValue<Vec<Event>, sec_lat::Label_Empty, int_lat::Label_All, DynLabel<Sec>, ()>>) -> SecureValue<Vec<Event>, sec_lat::Label_Empty, int_lat::Label_All, DynLabel<Sec>, ()> {
        let mut dyn_sec_label = DynLabel::<Sec>::default();
        for i in 0..events.len() {
            dyn_sec_label = dyn_sec_label.join(events[i].get_dyn_sec_label_ref());
        }
        untrusted_secure_block_dynamic_secrecy!(sec_lat::Label_Empty, int_lat::Label_All, &dyn_sec_label, {
            let mut flattened: Vec<Event> = std::vec::Vec::new();
            for event_set in events {
                flattened.extend(unwrap(event_set));
            }
            wrap(flattened)
        })
    }

    let events1: Vec<SecureValue<Vec<Event>, _, _, _, _>> = futures::future::join_all(tasks)
        .await
        .into_iter()
        .filter_map(|r| r.ok())
        .map(|r| r.unwrap())
    ...
    let events = secure_flatten_and_collect(events1);
    total_duration += AVAIL_TIMING.elapsed();

    pb.finish_with_message("Retrieved events.");
    let cur_s = total_duration.as_secs();
    let cur_ns = total_duration.subsec_nanos();

    let pb = m.add(ProgressBar::new(1));
    pb.set_message("Computing availabilities...");
    pb.enable_steady_tick(Duration::milliseconds(250).to_std().unwrap());

    let mut AVAIL_TIMING_2: Instant = Instant::now();
    let sec_availability = finder.get_availability(events, &pb)?;
    let availability = trusted_secure_block_dynamic_secrecy!(sec_lat::Label_Empty, int_lat::Label_All, &sec_availability.get_dyn_sec_label_ref(), {
        unwrap(sec_availability)
    });

    total_duration += AVAIL_TIMING_2.elapsed();
    let string_timing = format!("First time: {} s, {} ns\nElapsed time: {} s, {} ns", cur_s, cur_ns, total_duration.as_secs(), total_duration.subsec_nanos());
    fs::write("timing.txt", string_timing).expect("Unable to write file");
    ...
    //CARAPACE NOTE: MultiSelect with ColorfulTheme is safe to be marked ISEF, being composed entirely of ISEF types
    ...
    let selected_slots = selection.into_iter().map(|i| { 
        slots.get(i).unwrap() 
    });
