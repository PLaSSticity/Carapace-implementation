...
use secret_structs::ternary_lattice::ternary_lattice as lat3;
use secret_structs::secret as st;

pub struct Example<T, L>
where
    T: st::SecureValueSafe,
{
    val: T,
...
pub fn inc<L, T>(x: &mut st::Secret<i32, L>, q: &MyDerefable<i32>) {
    ...
    secret_structs::secret_block!(L {
        ...
        let y: i32 = *z;
        //let q2 = q;
        let y = unwrap_mut_ref(x);
        *y = 1;
    });
}

pub fn sort<T: st::SecureValueSafe + Ord, L>(myvec: &mut st::Secret<Vec<T>, L>) {
    secret_structs::secret_block!(L {
        let x = unwrap_mut_ref(myvec);
        <[T]>::sort(x);
    });
}
...
fn main() {
    ...
    */
    ...
    // define secrets
    let sec = st::Secret::<i64, lat3::Label_ABC>::new(42);
    let med = st::Secret::<i64, lat3::Label_AB>::new(25);
    let not_sec = st::Secret::<i64, lat3::Label_Empty>::new(3);
    ...
    let x: st::Secret<i64, lat3::Label_ABC> = st::apply_binary_ref(add_closure, &sec, &med);
    ...
    // let y1 : st::Secret::<i64, lat3::Label_AB> = st::apply_binary_ref(add_closure, &not_sec, &med);
    // let y2 : st::Secret::<i64, lat3::Label_AB> = st::apply_binary_ref(add_closure, &not_sec, &med);
    ...
    // define secrets
    let sec_bool = st::Secret::<bool, lat3::Label_ABC>::new(true);
    let _med_bool = st::Secret::<bool, lat3::Label_A>::new(false);
    let not_sec_bool = st::Secret::<bool, lat3::Label_Empty>::new(true);

    // define and closure
    let and_closure =
        secret_macros::secret_closure!(|op: (&bool, &bool)| -> bool { *op.0 && *op.1 });

    let xx: st::Secret<bool, lat3::Label_ABC> =
    ...
    // define secrets
    let _sec_string = st::Secret::<String, lat3::Label_ABC>::new(String::from("Hello, world! "));
    let med_string = st::Secret::<String, lat3::Label_A>::new(String::from("Goodbye, cruel world. "));
    let not_sec_string = st::Secret::<String, lat3::Label_Empty>::new(String::from(
    ...
    let sec_vec = st::Secret::<Vec<i32>, lat3::Label_ABC>::new(vec![1, 2, 3]);
    let med_vec = st::Secret::<Vec<i32>, lat3::Label_AB>::new(vec![4, 5, 6]);
    let not_sec_vec = st::Secret::<Vec<i32>, lat3::Label_Empty>::new(vec![7, 8, 9]);
    // Only mutable because we currently don't have an apply() which takes (&mut, &)
    let _secret_val = st::Secret::<i32, lat3::Label_AB>::new(5);
    ...
    let struct1 = Example {
        val: 5,
        secret_val: st::Secret::<i64, lat3::Label_ABC>::new(10),
    };
    let struct2 = Example {
        val: 2,
        secret_val: st::Secret::<i64, lat3::Label_AB>::new(4),
    };
    let struct3 = Example {
        val: 1,
        secret_val: st::Secret::<i64, lat3::Label_AB>::new(2),
    };
    let _sec_struct = st::Secret::<Example<i64, lat3::Label_ABC>, lat3::Label_ABC>::new(struct1);
    let med_struct = st::Secret::<Example<i64, lat3::Label_AB>, lat3::Label_AB>::new(struct2);
    let not_sec_struct = st::Secret::<Example<i64, lat3::Label_AB>, lat3::Label_Empty>::new(struct3);
    ...
    //let my_arc = st::Secret::<Rc<i64>, lat3::Label_AB>::new(Rc::<i64>::new(5));  // doesn't compile - fails SecureValueSafe
    ...
    *Rc::make_mut(&mut private_rc2) = 6; // Clones private_rc2 so this does not affect public_rc1
    ...
    // let private_mutex = st::Secret::<Mutex<i64>, lat3::Label_AB>::new(public_mutex.clone());    // Doesn't compile since Mutex uses UnsafeCell

    // let private_mutex_wrapped = st::Secret::<Arc<Mutex<i64>>, lat3::Label_AB>::new(Arc::<Mutex<i64>>::new(Mutex::new(1)));  // Doesn't compile - fails SecureValueSafe
}
